"""
Project for Week 4 of "Python Data Analysis".
Processing CSV files with baseball stastics.

Be sure to read the project description page for further information
about the expected behavior of the program.
"""

import csv

##
## Provided code from Week 3 Project
##

def read_csv_as_list_dict(filename, separator, quote):
    """
    Inputs:
      filename  - name of CSV file
      separator - character that separates fields
      quote     - character used to optionally quote fields
    Output:
      Returns a list of dictionaries where each item in the list
      corresponds to a row in the CSV file.  The dictionaries in the
      list map the field names to the field values for that row.
    """
    table = []
    with open(filename, newline='') as csvfile:
        csvreader = csv.DictReader(csvfile, delimiter=separator, quotechar=quote)
        for row in csvreader:
            table.append(row)
    return table


def read_csv_as_nested_dict(filename, keyfield, separator, quote):
    """
    Inputs:
      filename  - name of CSV file
      keyfield  - field to use as key for rows
      separator - character that separates fields
      quote     - character used to optionally quote fields
    Output:
      Returns a dictionary of dictionaries where the outer dictionary
      maps the value in the key_field to the corresponding row in the
      CSV file.  The inner dictionaries map the field names to the
      field values for that row.
    """
    table = {}
    with open(filename, newline='') as csvfile:
        csvreader = csv.DictReader(csvfile, delimiter=separator, quotechar=quote)
        for row in csvreader:
            rowid = row[keyfield]
            table[rowid] = row
    return table

##
## Provided formulas for common batting statistics
##

# Typical cutoff used for official statistics
MINIMUM_AB = 500

def batting_average(info, batting_stats):
    """
    Inputs:
      batting_stats - dictionary of batting statistics (values are strings)
    Output:
      Returns the batting average as a float
    """
    hits = float(batting_stats[info["hits"]])
    at_bats = float(batting_stats[info["atbats"]])
    if at_bats >= MINIMUM_AB:
        return hits / at_bats
    else:
        return 0

def onbase_percentage(info, batting_stats):
    """
    Inputs:
      batting_stats - dictionary of batting statistics (values are strings)
    Output:
      Returns the on-base percentage as a float
    """
    hits = float(batting_stats[info["hits"]])
    at_bats = float(batting_stats[info["atbats"]])
    walks = float(batting_stats[info["walks"]])
    if at_bats >= MINIMUM_AB:
        return (hits + walks) / (at_bats + walks)
    else:
        return 0

def slugging_percentage(info, batting_stats):
    """
    Inputs:
      batting_stats - dictionary of batting statistics (values are strings)
    Output:
      Returns the slugging percentage as a float
    """
    hits = float(batting_stats[info["hits"]])
    doubles = float(batting_stats[info["doubles"]])
    triples = float(batting_stats[info["triples"]])
    home_runs = float(batting_stats[info["homeruns"]])
    singles = hits - doubles - triples - home_runs
    at_bats = float(batting_stats[info["atbats"]])
    if at_bats >= MINIMUM_AB:
        return (singles + 2 * doubles + 3 * triples + 4 * home_runs) / at_bats
    else:
        return 0




##
## Part 1: Functions to compute top batting statistics by year
##




def filter_by_year(statistics, year, yearid):
    """
    Inputs:
      statistics - List of batting statistics dictionaries
      year       - Year to filter by
      yearid     - Year ID field in statistics
    Outputs:
      Returns a list of batting statistics dictionaries that
      are from the input year.
    """

    # Establish output reference.
    year_list = []

    # Filter dictionaries in "statistics" which meet criteria of "year"
    # argument matching "yearid" argument and submitting them to the output
    # reference.
    for each in statistics:
        if each[yearid] == str(year):
            year_list.append(each)

    return year_list




def top_player_ids(info, statistics, formula, numplayers):
    """
    Inputs:
      info       - Baseball data information dictionary
      statistics - List of batting statistics dictionaries
      formula    - function that takes an info dictionary and a
                   batting statistics dictionary as input and
                   computes a compound statistic
      numplayers - Number of top players to return
    Outputs:
      Returns a list of tuples, player ID and compound statistic
      computed by formula, of the top numplayers players sorted in
      decreasing order of the computed statistic.
    """

    # Establish output reference.
    player_stat_list = []

    # For each dictionary in "statistics" argument the for loop runs the argued
    # "formula" on "info" and "statistics" to establish the "stat" then tuple
    # it with the dictionary's "playerid".
    for each in statistics:
        stat = formula(info, each)
        player_tup = (each[info["playerid"]], stat)
        player_stat_list.append(player_tup)

    # Sort the "player_stat_list" from highest "stat".
    player_stat_list.sort(reverse=True, key=lambda group: group[1])

    # Return on argued "numplayers".
    return player_stat_list[:numplayers]




def lookup_player_names(info, top_ids_and_stats):
    """
    Inputs:
      info              - Baseball data information dictionary
      top_ids_and_stats - list of tuples containing player IDs and
                          computed statistics
    Outputs:
      List of strings of the form "x.xxx --- FirstName LastName",
      where "x.xxx" is a string conversion of the float stat in
      the input and "FirstName LastName" is the name of the player
      corresponding to the player ID in the input.
    """

    # Establish output reference.
    stat_strings = []
    # Establish reference list of csv file.
    stat_list = []

    # Temporarily open csv file from arguments with "DictReader".
    with open(info["masterfile"]) as csv_file:
        read = csv.DictReader(csv_file, delimiter=info["separator"], \
        quotechar=info["quote"])

        # Iterate over csv file to reference "name_list".
        for line in read:
            stat_list.append(line)

    # Iterate over "top_ids_and_stats" argument.
    for each in top_ids_and_stats:
        first_name = ""
        last_name = ""

        # Iterate over "stat_list".
        for keys in stat_list:
            # Compare playerid from "top_ids_and_stats" to playerid from
            # csv file through "keys" from "info" dictionary to establish
            # exact "first_name" and "last_name" of player.
            if each[0] == keys[info["playerid"]]:
                first_name = keys[info["firstname"]]
                last_name = keys[info["lastname"]]

        # Turns stat from "top_ids_list" into specific string.
        stat = "{0:.3f}".format(each[1])
        # Turns all input from iterated "top_ids_and_stats" into desired
        # string outcome.
        string = stat + " --- " + first_name + " " + last_name
        # For multiple "top_ids_and_stats" appends "string" to complete
        # list.
        stat_strings.append(string)

    return stat_strings




def compute_top_stats_year(info, formula, numplayers, year):
    """
    Inputs:
      info        - Baseball data information dictionary
      formula     - function that takes an info dictionary and a
                    batting statistics dictionary as input and
                    computes a compound statistic
      numplayers  - Number of top players to return
      year        - Year to filter by
    Outputs:
      Returns a list of strings for the top numplayers in the given year
      according to the given formula.
    """

    # Open csv file.
    csv_stats = read_csv_as_list_dict(info["battingfile"], info["separator"], info["quote"])

    # Turn csv file into list of dictionaries.
    dict_stats = []
    for row in csv_stats:
        dict_stats.append(dict(row))

    # Filter file to only give argued year.
    year_stats = []
    for row in dict_stats:
        if row[info["yearid"]] == str(year):
            year_stats.append(row)

    # Sort file in order.
    year_stats_sort = top_player_ids(info, year_stats, formula, numplayers)

    # Select argued number of returns from sorted file.
    top_stats = lookup_player_names(info, year_stats_sort)

    return top_stats




##
## Part 2: Functions to compute top batting statistics by career
##




def aggregate_by_player_id(statistics, playerid, fields):
    """
    Inputs:
      statistics - List of batting statistics dictionaries
      playerid   - Player ID field name
      fields     - List of fields to aggregate
    Output:
      Returns a nested dictionary whose keys are player IDs and whose values
      are dictionaries of aggregated stats.  Only the fields from the fields
      input will be aggregated in the aggregated stats dictionaries.
    """

    # Establish output reference.
    agg_dict = {}

    # Start iteration over argued "statistics" starting with each dictionary in "statistics".
    for row in statistics:
        # Establish temporary dictionary to be loaded to output dictionary.
        new_row = {}
        # Iterate over argued "fields" within.
        for each in fields:
            # Verify that "each" in "fields" is in "row" in "statistics".  If
            # confirmed then load "each" in "row" into "new_row".
            if each in row:
                new_row[each] = int(row[each])

        # Load "playerid" from "row" into "new_row".
        new_row[playerid] = row[playerid]

        # Perform "if" / "else" comparisons to check if the current created
        # "new_row" is already within the output reference of "agg_dict".  If
        # not it is added.  If it is, then the stats within are summed.
        if row[playerid] not in agg_dict:
            agg_dict[row[playerid]] = new_row
        else:
            for each in fields:
                agg_dict[row[playerid]][each] = agg_dict[row[playerid]][each] \
                + new_row[each]

    return agg_dict




def compute_top_stats_career(info, formula, numplayers):
    """
    Inputs:
      info        - Baseball data information dictionary
      formula     - function that takes an info dictionary and a
                    batting statistics dictionary as input and
                    computes a compound statistic
      numplayers  - Number of top players to return
    """

    # Open csv file.
    csv_stats = read_csv_as_list_dict(info["battingfile"], info["separator"], info["quote"])

    # Turn csv file into list of dictionaries.
    dict_stats = []
    for row in csv_stats:
        dict_stats.append(dict(row))

    # Establish transferring dictionary "agg_dict" and make temporary reference
    # "player" to save key strokes.
    agg_dict = {}
    player = info["playerid"]
    # Start iteration over argued "statistics" starting with each dictionary in "statistics".
    for row in dict_stats:
        # Establish temporary dictionary to be loaded to transferring
        # dictionary.
        new_row = {}
        # Iterate over argued "fields" within.
        for each in info["battingfields"]:
            # Verify that "each" in "fields" is in "row" in "statistics".  If
            # confirmed then load "each" in "row" into "new_row".
            if each in row:
                new_row[each] = int(row[each])

        # Load "playerid" from "row" into "new_row".
        new_row[player] = row[player]

        # Perform "if" / "else" comparisons to check if the current created
        # "new_row" is already within the output reference of "agg_dict".  If
        # not it is added.  If it is, then the stats within are summed.
        if row[player] not in agg_dict:
            agg_dict[row[player]] = new_row
        else:
            for each in info["battingfields"]:
                agg_dict[row[player]][each] = agg_dict[row[player]][each] \
                + new_row[each]

    # Establish temporary data transfer reference.
    player_career_stat_list = []

    # Perform argued "formula" over iterated data "agg_dict".  Then create
    # temporary list of tuples to transfer to final output for "string" format.
    for each in agg_dict:
        stat = formula(info, agg_dict[each])
        player_tup = (each, stat)
        player_career_stat_list.append(player_tup)

    # Perform "sort" on "player_career_stat_list" and designate argued
    # "numplayers" for top player selection.
    player_career_stat_list.sort(reverse=True, key=lambda group: group[1])
    player_career_stat_list = player_career_stat_list[:numplayers]

    # Perform function "lookup_player_names" to turn final output in to desired
    # string format.
    final_list = lookup_player_names(info, player_career_stat_list)

    return final_list




##
## Provided testing code
##

def test_baseball_statistics():
    """
    Simple testing code.
    """

    #
    # Dictionary containing information needed to access baseball statistics
    # This information is all tied to the format and contents of the CSV files
    #
    baseballdatainfo = {"masterfile": "Master_2016.csv",   # Name of Master CSV file
                        "battingfile": "Batting_2016.csv", # Name of Batting CSV file
                        "separator": ",",                  # Separator character in CSV files
                        "quote": '"',                      # Quote character in CSV files
                        "playerid": "playerID",            # Player ID field name
                        "firstname": "nameFirst",          # First name field name
                        "lastname": "nameLast",            # Last name field name
                        "yearid": "yearID",                # Year field name
                        "atbats": "AB",                    # At bats field name
                        "hits": "H",                       # Hits field name
                        "doubles": "2B",                   # Doubles field name
                        "triples": "3B",                   # Triples field name
                        "homeruns": "HR",                  # Home runs field name
                        "walks": "BB",                     # Walks field name
                        "battingfields": ["AB", "H", "2B", "3B", "HR", "BB"]}

    print("Top 5 batting averages in 1923")
    top_batting_average_1923 = compute_top_stats_year(baseballdatainfo, batting_average, 5, 1923)
    for player in top_batting_average_1923:
        print(player)
    print("")

    print("Top 10 batting averages in 2010")
    top_batting_average_2010 = compute_top_stats_year(baseballdatainfo, batting_average, 10, 2010)
    for player in top_batting_average_2010:
        print(player)
    print("")

    print("Top 10 on-base percentage in 2010")
    top_onbase_2010 = compute_top_stats_year(baseballdatainfo, onbase_percentage, 10, 2010)
    for player in top_onbase_2010:
        print(player)
    print("")

    print("Top 10 slugging percentage in 2010")
    top_slugging_2010 = compute_top_stats_year(baseballdatainfo, slugging_percentage, 10, 2010)
    for player in top_slugging_2010:
        print(player)
    print("")

    # You can also use lambdas for the formula
    #  This one computes onbase plus slugging percentage
    print("Top 10 OPS in 2010")
    top_ops_2010 = compute_top_stats_year(baseballdatainfo,
                                          lambda info, stats: (onbase_percentage(info, stats) +
                                                               slugging_percentage(info, stats)),
                                          10, 2010)
    for player in top_ops_2010:
        print(player)
    print("")

    print("Top 20 career batting averages")
    top_batting_average_career = compute_top_stats_career(baseballdatainfo, batting_average, 20)
    for player in top_batting_average_career:
        print(player)
    print("")


# Make sure the following call to test_baseball_statistics is
# commented out when submitting to OwlTest/CourseraTest.

# test_baseball_statistics()
